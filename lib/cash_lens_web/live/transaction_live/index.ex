defmodule CashLensWeb.TransactionLive.Index do
  ...
  def render(assigns) do
    ...
    <div class="py-6 space-y-8">
      ...
      <div class="flex items-center justify-between">
        <div class="join">
          <button 
            ...
            class={["join-item btn btn-sm px-4", if(@filters["type"] == "debit", do: "btn-error", else: "btn-outline border-base-300")]}
          >
          ...
          <button 
            ...
            class={["join-item btn btn-sm px-4", if(@filters["type"] == "credit", do: "btn-success", else: "btn-outline border-base-300")]}
          >
            <.icon name="hero-arrow-trending-up" class="size-4 mr-1" /> Créditos
          </button>
        </div>

        <div :if={@filters["type"] != ""} class="text-[10px] uppercase font-black opacity-40 flex items-center gap-1 bg-base-200 px-3 py-1.5 rounded-full border border-base-300 animate-in fade-in zoom-in duration-200">
          <.icon name="hero-funnel" class="size-3" />
          Mostrando apenas {@filters["type"] == "debit" && "Débitos" || "Créditos"}
        </div>
      </div>
        ...
        <div class="stats shadow bg-base-100 border border-base-300">
          <div class="stat">
            <div class="stat-title text-success flex items-center gap-1">
              <.icon name="hero-arrow-up-circle" class="size-3" />
        ...
        <div class="stats shadow bg-base-100 border border-base-300">
          <div class="stat">
            <div class="stat-title text-error flex items-center gap-1">
              <.icon name="hero-arrow-down-circle" class="size-3" />
        ...
        <div class="stats shadow bg-base-100 border border-base-300">
          <div class="stat">
            <div class="stat-title font-bold flex items-center gap-1">
              ...
              Balanço ({@summary.month_name})
            </div>
            <% balance = Decimal.sub(@summary.income, @summary.expenses) %>
            <div class={["stat-value font-black text-2xl", if(Decimal.gt?(balance, 0), do: "text-success", else: "text-error")]}>
      ...
      <.modal :if={@show_balance_correction} id="balance-correction-modal" show on_cancel={JS.push("close_modal")}>
        <div class="p-2">
          ...
          <.form :let={f} for={@balance_correction_form} id="balance-correction-form" phx-submit="save_balance_correction" class="space-y-6">
            <div class="space-y-1">
              <label class="block text-xs font-black uppercase opacity-40">Saldo Atual Registrado</label>
              <div class="text-xl font-mono font-bold">{format_currency(@summary.current_balance)}</div>
            </div>
            ...
            <div class="space-y-2">
              ...
              <div :if={!Decimal.eq?(@balance_diff, 0)} class="flex items-center gap-2 px-3 py-2 bg-base-200 rounded-lg border border-base-300 animate-in fade-in slide-in-from-top-1 duration-200">
                <span class="text-[10px] font-black uppercase opacity-50 text-nowrap">Diferença (Ajuste):</span>
                <span class={["font-mono font-bold text-sm", if(Decimal.gt?(@balance_diff, 0), do: "text-success", else: "text-error")]}>
            ...
            <div class="space-y-3">
              ...
              <div class="flex flex-col gap-2">
                <label class="flex items-center gap-3 p-3 border border-base-300 rounded-xl cursor-pointer hover:bg-base-200 transition-all has-[:checked]:border-primary has-[:checked]:bg-primary/5">
                  <input type="radio" name="adjustment_type" value="rendimentos" class="radio radio-primary" checked />
                  <div>
                    <span class="block font-bold text-sm">Rendimentos</span>
                    <span class="block text-[10px] opacity-60">Cria uma nova transação com a diferença de valor.</span>
                  </div>
                </label>
                <label class="flex items-center gap-3 p-3 border border-base-300 rounded-xl cursor-pointer hover:bg-base-200 transition-all has-[:checked]:border-primary has-[:checked]:bg-primary/5">
                  <input type="radio" name="adjustment_type" value="ajuste_inicial" class="radio radio-primary" />
                  <div>
                    <span class="block font-bold text-sm">Ajuste Inicial</span>
                    <span class="block text-[10px] opacity-60">Altera o saldo do balanço mais antigo e recalcula o histórico.</span>
                  </div>
                </label>
              </div>
            </div>
            ...
            <div class="pt-2">
              <.button phx-disable-with="Ajustando..." variant="primary" class="w-full">Confirmar Ajuste</.button>
            </div>
          </.form>
        </div>
      </.modal>
                ...
                <tr>
                  ...
                  <th class="w-32 px-4">
                    <div class="flex flex-col gap-1 text-right">
                      <span>Valor</span>
                      <input type="number" name="amount" value={@filters["amount"]} placeholder="0.00" step="any" class="input input-bordered input-xs font-normal w-full text-right" phx-debounce="300" />
                    </div>
                  </th>
                  <th class="w-48 px-4">
                    <div class="flex flex-col gap-1">
            ...
            <tbody id="transactions" phx-update="stream" class="overflow-visible">
              <tr :for={{id, transaction} <- @streams.transactions} id={id} class="hover group border-b border-base-200 overflow-visible">
                ...
                <td class="py-2 px-4 truncate max-w-xs md:max-w-md">
                  <div class="flex flex-col">
                    <div class="leading-relaxed font-medium truncate">{transaction.description}</div>
                ...
                <td class="w-48 px-4 overflow-visible">
                  <div class="flex items-center gap-1 relative overflow-visible">
                    <div id={"cat-select-#{transaction.id}"} phx-hook="CategoryAutocomplete" data-transaction-id={transaction.id} data-categories={Jason.encode!(Enum.map(@categories, &%{id: &1.id, name: CashLens.Categories.Category.full_name(&1)}))} class="relative w-full overflow-visible" phx-click-stop>
      ...
      <.modal :if={@confirm_modal} id="confirm-modal" show on_cancel={JS.push("close_modal")}>
        <div class="p-4 text-center">
          ...
          <p class="text-base-content/60 mb-10">Deseja realmente apagar esta transação? Esta ação não pode ser desfeita.</p>
          ...
          <form id="upload-form" phx-submit="save_import" phx-change="validate_import">
            ...
            <div class="form-control w-full mb-8">
              ...
              <div :for={entry <- @uploads.statement.entries} class="mt-4 p-3 bg-base-100 rounded-xl border border-base-300 flex items-center justify-between animate-in slide-in-from-left-2">
                ...
                <button type="button" phx-click={JS.push("lv:cancel-upload", value: %{ref: entry.ref, upload: "statement"})} class="btn btn-ghost btn-xs text-error">
      ...
      <.modal :if={@show_quick_category_modal} id="quick-category-modal" show on_cancel={JS.push("close_modal")}>
        <div class="p-2">
          <h2 class="text-2xl font-black mb-2 uppercase tracking-tighter text-primary">Nova Categoria</h2>
          <p class="text-xs opacity-60 mb-6">Crie uma nova categoria para organizar seus lançamentos.</p>

          <.form :let={f} for={@category_form} id="quick-category-form" phx-submit="save_quick_category" class="space-y-6">
            <.input field={f[:name]} type="text" label="Nome da Categoria" placeholder="Ex: Alimentação, Lazer..." required />
            
            <div class="form-control w-full">
              <label class="label"><span class="label-text font-bold">Categoria Pai (Opcional)</span></label>
            ...
            <div class="pt-2">
              <button type="submit" class="btn btn-primary btn-lg w-full rounded-2xl shadow-lg shadow-primary/20" phx-disable-with="Criando...">
                Salvar Categoria
              </button>
            </div>
          </.form>
      ...
      <.modal :if={@bulk_confirmation} id="bulk-modal" show on_cancel={JS.push("close_modal")}>
        <div class="p-2">
          ...
          <p class="text-sm opacity-70 mb-6">
            Encontramos mais <strong>{length(@bulk_confirmation.items)}</strong> transações com a descrição 
            <span class="font-mono bg-base-300 px-1 rounded">"{@bulk_confirmation.description}"</span>. 
            Deseja categorizar todas como <strong class="text-primary">{@bulk_confirmation.category_name}</strong>?
          </p>
          
          <div class="flex flex-col sm:flex-row gap-3">
            <button phx-click="apply_bulk_category" class="btn btn-primary btn-lg flex-1 rounded-2xl shadow-lg shadow-primary/20">Sim, Categorizar Tudo</button>
            <button phx-click="close_modal" class="btn btn-ghost btn-lg flex-1 rounded-2xl">Não, Apenas Esta</button>
          </div>
        </div>
      </.modal>
  ...
  def handle_event("delete", %{"id" => id}, socket) do
    ...
    {:noreply, socket |> assign(:confirm_modal, nil) |> stream_delete(:transactions, transaction) |> assign(:pending_count, Transactions.count_pending_transactions())}
  end

  @impl true
  def handle_event("delete_all", _params, socket) do
    Transactions.delete_all_transactions()
    {:noreply, socket |> assign(:confirm_modal, nil) |> stream(:transactions, [], reset: true) |> assign(:pending_count, 0)}
  end
  ...
  def handle_event("open_balance_correction", _params, socket) do
    ...
    {:noreply, 
     ...
     |> assign(:balance_diff, Decimal.new("0"))
     |> assign(:balance_correction_form, to_form(%{"new_balance" => Decimal.to_string(current_balance, :normal)}))}
  end
  ...
  def handle_event("update_diff", %{"value" => value}, socket) do
    ...
    new_val = if value in ["", "-", ".", "-."], do: Decimal.new("0"), else: (Decimal.parse(value) |> case do {d, _} -> d; _ -> Decimal.new("0") end)
  ...
  def handle_event("save_balance_correction", %{"new_balance" => new_balance, "adjustment_type" => type}, socket) do
    ...
    case type do
      "rendimentos" ->
        ...
        category = case Categories.get_category_by_slug("rendimentos") do
          nil -> 
            {:ok, cat} = Categories.create_category(%{name: "Rendimentos", slug: "rendimentos", type: "variable"})
            cat
          cat -> cat
        end
      ...
      "ajuste_inicial" ->
        ...
        if oldest do
          ...
          Accounts.update_account(account, %{balance: Decimal.add(account.balance || Decimal.new("0"), diff)})
        end
    end
